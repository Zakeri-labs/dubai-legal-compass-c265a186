...
export function FloatingWhatsApp() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mobile-menu-open-change", onMenuOpenChange);
    return () => window.removeEventListener("mobile-menu-open-change", onMenuOpenChange);
  }, []);
  ...
  return (
    <a
      ...
      className={`group fixed bottom-20 right-4 z-40 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/20 transition-all duration-300 hover:scale-105 lg:bottom-6 lg:right-6 ${
        visible && !menuOpen ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-4 opacity-0"
      }`}
    >
